...
def time_features(dates, freq='h'):
    ...
    return df.drop(['date'], axis=1).values.T  
...
class Dataset_pretrain(Dataset):
    ...
    def __read_data__(self):
        ...
        for pt_dataset in pt_datasets:
            ...
            # print(f"len(data_stamp): {len(data_stamp)}")
            # print(f"len(cols_data): {len(cols_data)}")
            num_columns = data_stamp.shape[1]
            df_stamp = np.array([data_stamp for i in range(len(cols_data))]).reshape((len(data_stamp) * len(cols_data), num_columns))  # 
...
class Dataset_stock(Dataset):
    def __init__(self, configs, root_path, flag='train', size=None,
                 features='MS', data_path='hs300_1.csv',
                 target='OT', scale=False, timeenc=1, freq='h', percent=100):
    ...
    def __read_data__(self):
        ...
        self.feature_cols=['close','open','high','low','prev_close','volume'] 
        self.full_cols =  ['close','open','high','low','prev_close','volume','OT'] 
        self.dataset = process_features(df, self.feature_cols)
        ...
        if self.data_name == "hs300" or self.data_name == "zz500":
            first_date ='2023-11-17' 
            last_date = '2024-12-31'
        else:
            first_date ='2023-11-14'
            last_date = '2024-12-31'
        border1s = [pd.to_datetime('2018-01-04'), pd.to_datetime('2023-01-04'), pd.to_datetime(first_date)]
        border2s = [pd.to_datetime('2022-12-29'), pd.to_datetime('2023-11-29'), pd.to_datetime(last_date)]
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]
        
        for stock in self.stocks:
            stock_data = self.dataset[self.dataset['kdcode'] == stock] 
            stock_data.index = pd.to_datetime(stock_data['dt'])

            start_index = stock_data.index.get_loc(border1)
            end_index = stock_data.index.get_loc(border2)

            data_x = stock_data.iloc[start_index:end_index+1].copy()
            data_y = stock_data.iloc[start_index:end_index+1].copy()
            
            if self.features == 'M' or self.features == 'MS':
                cols_data = self.full_cols
                feature_data = data_x[cols_data]
                label_data = data_y[cols_data]
            elif self.features == 'S':
                feature_data = data_x[[self.target]]
                label_data = data_y[[self.target]]

            df_stamp = data_x[['dt']].set_index('dt')
            df_stamp = df_stamp.loc[border1:border2]
            df_stamp.reset_index(inplace=True)

            if self.timeenc == 0:
                df_stamp['year'] = df_stamp['dt'].apply(lambda row: row.year)
                df_stamp['month'] = df_stamp['dt'].apply(lambda row: row.month)
                df_stamp['day'] = df_stamp['dt'].apply(lambda row: row.day)
                data_stamp = df_stamp.drop(['dt'], axis=1).values
            elif self.timeenc == 1:
                data_stamp = time_features(pd.to_datetime(df_stamp['dt'].values), freq=self.freq)
                data_stamp = data_stamp.transpose(1, 0)
            ...
            for i in range(len(data_x)-self.seq_len-self.pred_len+1):
                ...
                f_slice = feature_data.iloc[s_begin:s_end].values
                l_slice = label_data.iloc[r_begin:r_end].values
                temp1.append(f_slice)
                temp2.append(l_slice)
            self.feature.append(temp1)
            self.label.append(temp2)

        self.feature = np.array(self.feature)
        self.label = np.array(self.label)  
        self.feature = np.transpose(self.feature,(1,0,2,3))  # (N, nstock, seq_len, nvars)
        self.label = np.transpose(self.label,(1,0,2,3))      # (N, nstock, label_len + pred_len, nvars)

        return self.feature, self.label, self.data_stamp
    ...
    def __getitem__(self, index):
        ...
        seq_x = self.feature[s_begin]
        seq_y = self.label[s_begin]
        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]
