...
if [ ! -d "/mnt/petrelfs/chengdawei/lustre/wavelettest/checkpoints/long-term-forecasting" ]; then
    mkdir /mnt/petrelfs/chengdawei/lustre/wavelettestS/checkpoints/long-term-forecasting
fi

if [ ! -d "/mnt/petrelfs/chengdawei/lustre/wavelettest/logs/long-term-forecasting/traffic" ]; then
    mkdir /mnt/petrelfs/chengdawei/lustre/wavelettest/logs/long-term-forecasting/traffic
fi

if [ ! -d "/mnt/petrelfs/chengdawei/lustre/wavelettest/checkpoints/long-term-forecasting/traffic" ]; then
    mkdir /mnt/petrelfs/chengdawei/lustre/wavelettest/checkpoints/long-term-forecasting/traffic
fi
...
do
    ...
    --master_addr="127.0.0.1" --master_port=10023 \
    --max_restarts=1 \
    /mnt/petrelfs/chengdawei/lustre/wavelettest/run_LLM4TS.py \
    --is_training 1 \
    ...
    --history_len 336 \
    --use_multi_gpu \
    --checkpoints /mnt/petrelfs/chengdawei/lustre/wavelettest/checkpoints/long-term-forecasting/$data_name \
    > /mnt/petrelfs/chengdawei/lustre/wavelettest/logs/long-term-forecasting/$data_name/$model_id'_336_720_llml'$llm_layers'_lr'$lr'_bs'$bs'_'$exp_des.log 2>&1 
done
done
done
done
