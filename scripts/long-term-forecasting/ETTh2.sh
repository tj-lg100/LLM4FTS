...
if [ ! -d "/mnt/petrelfs/chengdawei/lustre/wavelettest/logs/long-term-forecasting/ETTh2" ]; then
    mkdir /mnt/petrelfs/chengdawei/lustre/wavelettest/logs/long-term-forecasting/ETTh2
fi

if [ ! -d "/mnt/petrelfs/chengdawei/lustre/wavelettest/checkpoints/long-term-forecasting/ETTh2" ]; then
    mkdir /mnt/petrelfs/chengdawei/lustre/wavelettest/checkpoints/long-term-forecasting/ETTh2
fi
...
do
    ...
    --history_len 336 \
    --checkpoints /mnt/petrelfs/chengdawei/lustre/wavelettest/checkpoints/long-term-forecasting/ETTh2 \
    > /mnt/petrelfs/chengdawei/lustre/wavelettest/logs/long-term-forecasting/ETTh2/$model_id'_336_'$pred_len'_llml'$llm_layers'_lr'$lr'_bs'$bs'_'$exp_des.log 2>&1 
done
done
done
done
