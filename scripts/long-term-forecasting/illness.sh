...
if [ ! -d "/mnt/petrelfs/chengdawei/lustre/wavelettest/logs/long-term-forecasting/illness" ]; then
    mkdir /mnt/petrelfs/chengdawei/lustre/wavelettest/logs/long-term-forecasting/illness
fi

if [ ! -d "/mnt/petrelfs/chengdawei/lustre/wavelettest/checkpoints/long-term-forecasting/illness" ]; then
    mkdir /mnt/petrelfs/chengdawei/lustre/wavelettest/checkpoints/long-term-forecasting/illness
fi
...
do
    ...
    --history_len 60 \
    --checkpoints /mnt/petrelfs/chengdawei/lustre/wavelettest/checkpoints/long-term-forecasting/illness \
    > /mnt/petrelfs/chengdawei/lustre/wavelettest/logs/long-term-forecasting/illness/$model_id'_336_720_llml'$llm_layers'_lr'$lr'_bs'$bs'_'$exp_des.log 2>&1 
done
done
done
done
