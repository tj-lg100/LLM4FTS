...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.wavename = configs.wavename
        ...
        if configs.wavelet:
            if "dwt" == configs.wavename:
                self.wave = 'db4' 
                self.J = 4  
                self.wt = DWT1DForward(wave=self.wave, J=self.J,mode ='periodization')
                self.iwt = DWT1DInverse(wave=self.wave, mode ='periodization')

                self.wt.train()
                self.iwt.train()
            elif "swt" == configs.wavename:
                self.wave = 'db4' 
                self.J = 3  
            elif "ldwt" == configs.wavename:
                self.wave = 'db4' 
                self.ldwt = WaveletTransform(in_channels =self.enc_in ,wavelet = self.wave)
                self.ldwt.train()
            else:
        ...
        self.in_layer_1 = nn.Linear(self.patch_len, configs.d_model)
        self.in_layer_2 = nn.Linear(self.patch_len, configs.d_model)
        ...
        for layer in (self.gpt, self.in_layer_1,self.in_layer_2, self.out_layer,self.pn):  # self.revin_layer
            layer.train()
    ...
    def forward(self, x, y, batch_index):
        ...
        # if self.wavelet and 'ldwt' == self.wavename:
        #     y = self.ldwt(y)
        ...
        if self.dynamic_segment:
            torch.save(x, f'/mnt/petrelfs/chengdawei/lustre/wavlet/tensors/origin_{self.dataset_name}/x_{batch_index}.pt')
            x = rearrange(x, 'b l m -> b m l')

            if self.wavelet and 'ldwt' == self.wavename:
                x_low,x_high = self.ldwt(x)

                x_low =  x_low.unfold(dimension=-1, size=self.patch_len, step=self.stride)  
                x_high =  x_high.unfold(dimension=-1, size=self.patch_len, step=self.stride)  

                x_low = rearrange(x_low, 'b m n p -> (b m) n p')
                x_high = rearrange(x_high, 'b m n p -> (b m) n p')

                ts_inputs_wave = torch.cat([self.in_layer_2(x_low),self.in_layer_2(x_high), self.in_layer_1(y)], dim=-2)

                my_attn_mask_wave = torch.ones((B*M, ts_inputs_wave.shape[-2], ts_inputs_wave.shape[-2])).to(ts_inputs_wave.device).bool()
                my_attn_mask_wave[:, :, -self.num_loss_patch:] = 0
                for i in range(-self.num_loss_patch, 0):
                    my_attn_mask_wave[:, i, i] = 1

                outputs_wave = self.gpt(inputs_embeds=ts_inputs_wave, attention_mask=my_attn_mask_wave).last_hidden_state # (bs*nstock*nvars, 3, d_model)
            ...
            x = torch.stack(x_patches, dim=2)  # (B,M, num_patches,  patch_len)
            
            x = rearrange(x, 'b m n p-> (b m) n p')  # (B*M, num_patches, patch_len)

            torch.save(x, f'/mnt/petrelfs/chengdawei/lustre/wavlet/tensors/patched_{self.dataset_name}/x_{batch_index}.pt')
            
            ts_inputs = torch.cat([self.in_layer_1(x), self.in_layer_1(y)], dim=-2)  # (B*M, num_patches, d_model)

        else:
            x = rearrange(x, 'b l m -> b m l')

            if self.wavelet and self.wavename=='ldwt':
                x_low,x_high = self.ldwt(x)
            ...
            ts_inputs = torch.cat([self.in_layer_1(x),self.in_layer_1(y)], dim=-2) # (bs*nstock*nvars, 3, d_model)
        ...
        if self.wavelet and 'ldwt' == self.wavename:
            outputs = torch.cat((outputs_wave, outputs), dim=1)  
