...
class Exp_Main(Exp_Basic):
    ...
    def _get_data(self, flag):
        #stock_dataset=self.args.stock_data
        #print(f"self.args.stock_data:{self.args.stock_data}")
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    if "LLM4TS_sft_zero" == self.args.model:
                        ...
                        if self.args.data_path == 'illness.csv':
                            ...
                            for kind in [(96 - self.args.patch_len) // self.args.stride + 1, (192 - self.args.patch_len) // self.args.stride + 1, (336 - self.args.patch_len) // self.args.stride + 1, (720 - self.args.patch_len) // self.args.stride + 1, (60 - self.args.patch_len) // self.args.stride + 1]:
                                if kind > self.args.pred_len:
                                    continue
                                loss += criterion(outputs[:, :, :kind, :], batch_y[:, :, :kind, :])
                                loss_num_dict={'96': 1., '192':2., '336': 3., '720': 4., '60': 5.}
                ...
                if self.args.lradj == 'TST':
                    ...
                    scheduler.step()
            print(self.model.ldwt.conv_low.weight.data)
            print(self.model.ldwt.conv_high.weight.data)
    ...
    def test(self, setting, test=0):
        ...
        if test:
            print('loading model')
            self.model.load_state_dict(torch.load(os.path.join('/mnt/petrelfs/chengdawei/lustre/wavlet/test_ckpt/' + setting[:-2], 'checkpoint.pth')))

        preds = []
        trues = []
        inputx = []
        folder_path = '/mnt/petrelfs/chengdawei/lustre/wavlet/test_results/' + setting + '/'
        ...
        folder_path = '/mnt/petrelfs/chengdawei/lustre/wavlet/results/' + setting + '/'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
